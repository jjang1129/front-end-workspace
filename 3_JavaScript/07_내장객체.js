function btn1() {
  ...
  console.log("절대값 : ", Math.abs(-8.8787878787)); // 8.878787878787
}
...
function btn3() {
  // console.log(Math.random()); // 0~1 숫자중 무작위
  //1 ~10 까지의 랜덤값
  const result = Math.floor(Math.random() * 10 + 1); //범위를 지정할때는 + = * %로 범위를 지정한다
  console.log(result);
}

function btn4() {
  const now = new Date(); //new Date는 생성 방식이 math랑 다르다
  console.log(now);

  console.log("년 : ", now.getFullYear()); // 2024
  console.log("월 : ", now.getMonth() + 1); // 5 , 0~11로 생각을한다 그래서 +1을 해줘야한다
...
function btn5() {
  // 년, 월, 일 ,시 ,분, 초
  const a = new Date(2024, 10, 13, 12, 50, 0); //11월로 하고싶으면 10월로 해야함 -1 , 초 삭제 가능
  console.log(a);
  // 년, 월, 일
